...
#include <lua.h>
#include <lauxlib.h>
    unsigned int texture_raw_glid(int);
}
...
{
    return (ImTextureID)(intptr_t)texture_raw_glid(lua_handle);
};
...
wSelectable(lua_State *L) {
	...
	case LUA_TNIL:
	case LUA_TBOOLEAN:
		selected = lua_toboolean(L, INDEX_ARGS) != 0;
...
wSetNextItemOpen(lua_State *L) {
	bool is_open = lua_toboolean(L, 1) != 0;
...
get_popup_args(lua_State *L, struct popup_args *args) {
	...
	if (lua_type(L, index) == LUA_TBOOLEAN) {
		args->b = lua_toboolean(L, index) != 0;
	} else {
		args->b = true;
	}
}
...
winSetNextWindowCollapsed(lua_State *L) {
	bool collapsed = lua_toboolean(L, 1) != 0;
...
uPushClipRect(lua_State *L) {
	...
	bool intersect_with_current_clip_rect = lua_toboolean(L, 5) != 0;
...
uSaveIniSettings(lua_State *L) {
	...
	ImGuiIO * io = &ImGui::GetIO();
	bool clear_want_save_flag = lua_toboolean(L, 1) != 0;
...
static int
fAddFont(lua_State *L) {
    const char* ttfPath = luaL_checkstring(L, 1);
    float fontSize = (float)luaL_optnumber(L, 2, 16.0f);
    const char* glyphRangeType = luaL_checkstring(L, 3);

    ImFontAtlas* atlas = ImGui::GetIO().Fonts;

    auto range = GetGlyphRanges(atlas, glyphRangeType);

    auto font = atlas->AddFontFromFileTTF(ttfPath, fontSize, NULL, range);
...
int
imgui_register(lua_State *L,const char* libname)
{
    luaL_requiref(L, libname, luaopen_imgui, 0);
    lua_pop(L, 1);
    return 1;
}
