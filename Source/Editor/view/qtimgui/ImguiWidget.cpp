...
ImguiWidget::ImguiWidget(QWidget* p)
	:CustomGLWidget(p)
{
    initialize();
    connect(&_timer, &QTimer::timeout,this,&ImguiWidget::onTimer);
    _timer.start(1000.0 / frameRate);
}
...
//关闭窗口时清理所有，避免内存泄漏
void ImguiWidget::closeEvent(QCloseEvent* event)
{ 
    clearAll(); 
}

void ImguiWidget::showEvent(QShowEvent* event)
{
    if (has_clear)
    {
        initialize();
...
void ImguiWidget::resizeEvent(QResizeEvent* event) 
{
    ...
    int w = width();
    int h = height();
    if (_fbo_pixels != nullptr)
    {
        delete [] _fbo_pixels;
        _fbo_pixels = nullptr;
    }
    unsigned int size = w * h * sizeof(unsigned char) * 4;
    _fbo_pixels = new unsigned char[size];
...
void ImguiWidget::fboToWidget()
{
    ...
    glBindTexture(GL_TEXTURE_2D, mirrorTexture);
    glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, &_fbo_pixels[0]);
    QImage tempimg(_fbo_pixels, this->width(), this->height(), QImage::Format::Format_RGBA8888);
    QImage final_img = tempimg.mirrored();
    QPalette palet;
    QBrush brush;
    brush.setTextureImage(final_img);
    palet.setBrush(QPalette::Window,brush);
    this->setPalette(palet);
    this->setAutoFillBackground(true);
   
}

ImTextureID ImguiWidget::getImg(const std::string& path) 
{
    ...
    _imgMap[path] = ImguiGLTexture(t_texId, imgsize, _elapse);
    return (ImTextureID)t_texId;
}

void ImguiWidget::checkImgCach()
{
    std::map<std::string, ImguiGLTexture> tmpImgMap;
    _curImgCach = 0.0;
...
void ImguiWidget::onMouseEvent(QMouseEvent* event)
{
...
void ImguiWidget::onKeyEvent(QKeyEvent* event)
{
    // Select current context
    ImGui::SetCurrentContext(g_ctx);
...
void ImguiWidget::updateCursorShape(const ImGuiIO& io)
{
#ifndef QT_NO_CURSOR
...
void ImguiWidget::setCursorPos(const ImGuiIO& io)
{
...
bool ImguiWidget::createFontsTexture()
{
    // Select current context
    ImGui::SetCurrentContext(g_ctx);
...
void ImguiWidget::deleteFonstTexture() 
{ 
    ImGuiIO& io = ImGui::GetIO();
...
bool ImguiWidget::createDevObjs()
{
    // Select current context
    if (_render_to_tex)
        createFbo();
...
void ImguiWidget::deleteDevObjs() 
{
    deleteFbo();
    if (g_VboHandle)
    {
