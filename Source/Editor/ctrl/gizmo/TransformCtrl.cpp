...
namespace urho3d
{
	namespace editor
	{
		...
		TransformCtrl::TransformCtrl(Context* ctx,eMode m,Node* gizmoRoot)
			:Object(ctx)
		{
			_gizmoRoot = gizmoRoot;
			mode = m;
			state = eCtrlState::eRunning;
			//modelRoot = NULL;
			object = nullptr;
			gizmo = nullptr;
			oprationEnable = true;
		...
		{
			if (state == eCtrlState::ePause || !object || _dragging || !oprationEnable)
				return;
			//line3d world_ray;
		...
		{
			if (state == eCtrlState::ePause || !object || _dragging)
			{
				return;
			}
			Ray world_ray = _camNode->GetComponent<Camera>()->GetScreenRay(x, y);
		...
		{
			if (!object || axis == "" || !_dragging || state == eCtrlState::ePause)
				return;
			if (!oprationEnable)
				return;
			switch (mode)
		...
		void TransformCtrl::attach(Node* obj)
		{
			...
			this->state = eCtrlState::eRunning;
		}
		...
		void TransformCtrl::setMode(eMode m)
		{
			if (mode == m)
				return;
			mode = m;
		...
		{
			auto targetState = isp == true ? eCtrlState::ePause : eCtrlState::eRunning;
			if (state == targetState)
				return;
			if (isp)
			{
				state = eCtrlState::ePause;
			}
			else
			{
				if (object)
